...
var min_error  = "<p class='warning alert alert-warning'>{l s='Please select an amount. Minimum amount to withdraw' mod='affiliates' js=1} : {convertPrice price=$MINIMUM_AMOUNT|escape:'htmlall':'UTF-8'|floatval}</p>";
					...
					<tbody>
						<tr id="total_rewards">
							...
							<td class="amount text-right nowrap">
								<strong>{convertPrice price=$total_rewards|escape:'htmlall':'UTF-8'|floatval}</strong>
							</td>
						</tr>
						<tr id="pending_rewards">
							<td class="text-right first_row">{l s='Pending Reward Amount' mod='affiliates'}</td>
							<td class="amount text-right nowrap">{convertPrice price=$pending_rewards|escape:'htmlall':'UTF-8'|floatval}</td>
						</tr>
						<tr id="paid_rewards">
							<td class="text-right first_row">{l s='Paid Amount' mod='affiliates'}</td>
							<td class="amount text-right nowrap">{convertPrice price=$total_paid|escape:'htmlall':'UTF-8'|floatval}</td>
						</tr>
						<tr id="awaiting_rewards">
							<td class="text-right first_row">{l s='Awaiting Payment Validation' mod='affiliates'}</td>
							<td class="amount text-right nowrap">{convertPrice price=$awaiting_payments|escape:'htmlall':'UTF-8'|floatval}</td>
						</tr>
						<tr id="reward_balance" class="{if isset($av_balance) AND $av_balance > 0}green_belt{else}low_balance{/if}">
							...
							<td class="amount text-right nowrap">
								<strong>{convertPrice price=$av_balance|escape:'htmlall':'UTF-8'|floatval}</strong>
							</td>
						</tr>
					</tbody>
				</table>
			...
			{if isset($PAYMENT_METHOD) AND $PAYMENT_METHOD}
				{if isset($av_balance) AND $av_balance}
					{if isset($MINIMUM_AMOUNT) AND $MINIMUM_AMOUNT > $av_balance}
						<p class="alert alert-warning warning">{l s='Minimum amount to withdraw' mod='affiliates'} : {convertPrice price=$MINIMUM_AMOUNT|escape:'htmlall':'UTF-8'|floatval}</p>
					{else}
							...
							<table class="{if $ps_version >= 1.6}affiliateion_table{/if} table table-bordered {if $ps_version < 1.6}std{/if}">
				                        ...
				                        <tr>
				                            ...
				                            <td>
				                                <span for="affiliate_groups_{$vreward.id_reward|escape:'htmlall':'UTF-8'}">
				                                	{convertPrice price=$vreward.reward_total|escape:'htmlall':'UTF-8'|floatval}
				                                </span>
				                            </td>
				                        </tr>
			                        {/foreach}
			                    ...
			                    <tfoot>
			                    	<tr>
			                    		...
			                    		<td class="title_box">
			                    			<label id="total_withdraw">{convertPrice price=0}</label>
