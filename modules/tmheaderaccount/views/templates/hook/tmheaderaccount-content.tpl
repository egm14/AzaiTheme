...
<ul class="menu-myaccount header-login-content toogle_content{if $is_logged} is-logged{/if}">
  {if $is_logged}
  <li {if $hook != 'left' && $hook != 'right'}class="{$configs.TMHEADERACCOUNT_DISPLAY_STYLE|escape:'quotes':'UTF-8'}"{/if}>
    <ul>
      <li class="user-data">
        ...
        <p><small>{l s='Welcome' mod='tmheaderaccount'}</small><br class="usuario"><span>{$firstname}</span> <span>{$lastname}</span></p>
      </li>
      <li>
    ...
    <p class="logout">
      <a class="btn btn-primary btn-md" href="{$link->getPageLink('index', true, NULL, "mylogout")|escape:'html'}" title="{l s='Sign out' mod='tmheaderaccount'}" rel="nofollow">
    ...
    <form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post">
      ...
      <div class="form_content clearfix">
        <div class="form-group">
          <input class="is_required validate account_input form-control email" data-validate="isEmail" type="text" name="header-email" placeholder="{l s='Email address' mod='tmheaderaccount'}" value="{if isset($smarty.post.email)}{$smarty.post.email|stripslashes|escape:'htmlall':'UTF-8'}{/if}"/>
        </div>
        <div class="form-group">
          <span>
            <input class="is_required validate account_input form-control password" type="password" data-validate="isPasswd" name="header-passwd" placeholder="{l s='Password' mod='tmheaderaccount'}" value="{if isset($smarty.post.passwd)}{$smarty.post.passwd|stripslashes|escape:'htmlall':'UTF-8'}{/if}" autocomplete="off"/>
          </span>
        </div>
        <p class="submit">
    ...
    <form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post" class="std">
      ...
      <div class="account_creation">
        ...
        <div class="required form-group">
          <input onkeyup="$('#firstname').val(this.value);" type="text" class="is_required validate form-control" data-validate="isName" name="firstname" placeholder="{l s='First name' mod='tmheaderaccount'}" value="{if isset($smarty.post.customer_firstname)}{$smarty.post.customer_firstname}{/if}"/>
        </div>
        <div class="required form-group">
          <input onkeyup="$('#lastname').val(this.value);" type="text" class="is_required validate form-control" data-validate="isName" name="lastname" placeholder="{l s='Last name' mod='tmheaderaccount'}" value="{if isset($smarty.post.customer_lastname)}{$smarty.post.customer_lastname}{/if}"/>
        </div>
        <div class="required form-group">
          <input type="email" class="is_required validate form-control" data-validate="isEmail" name="email" placeholder="{l s='Email' mod='tmheaderaccount'}" value="{if isset($smarty.post.email)}{$smarty.post.email}{/if}"/>
        </div>
        <div class="required password form-group">
          <input type="password" class="is_required validate form-control" data-validate="isPasswd" name="passwd" placeholder="{l s='Password' mod='tmheaderaccount'}"/>
          ...
          {foreach from=$dlv_all_fields item=field_name}
            ...
            {elseif $field_name eq "postcode"}
              ...
              <p class="required postcode form-group">
                ...
                <input type="text" class="form-control" name="postcode" id="postcode-tmha" data-validate="isPostCode" value="{if isset($smarty.post.postcode)}{$smarty.post.postcode}{/if}"/>
              </p>
    ...
    <form action="{$request_uri|escape:'html':'UTF-8'}" method="post" class="std">
      <fieldset>
        <div class="form-group">
          ...
          <div class="alert alert-danger" style="display:none;"></div>
          <input class="form-control" type="email" name="email" placeholder="{l s='Email address' mod='tmheaderaccount'}" value="{if isset($smarty.post.email)}{$smarty.post.email|escape:'html':'UTF-8'|stripslashes}{/if}"/>
        </div>
        <p class="submit">
