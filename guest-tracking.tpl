  ...
  {else}
    <form method="post" action="{$action|escape:'html':'UTF-8'}#guestToCustomer" class="std">
      <fieldset class="description_box box">
        ...
        <p>
          <button type="submit" name="submitTransformGuestToCustomer" class="btn-md btn btn-default icon-right">
            <span>
              {l s='Send'}
            </span>
          </button>
  ...
  <form method="post" action="{$action|escape:'html':'UTF-8'}" class="std" id="guestTracking">
    <fieldset class="description_box box">
      ...
      <div class="text form-group">
        ...
        <input placeholder="QIIXJXNUI" class="form-control" type="text" name="order_reference" value="{if isset($smarty.get.id_order)}{$smarty.get.id_order|escape:'html':'UTF-8'}{else}{if isset($smarty.post.id_order)}{$smarty.post.id_order|escape:'html':'UTF-8'}{/if}{/if}" size="8" />
        <i>{l s='Get the reference of the order on your invoice.'}</i>
      </div>
      <div class="text form-group">
        <label>{l s='Email:'}</label>
        <input placeholder="Email@example.com" class="form-control" type="email" name="email" value="{if isset($smarty.get.email)}{$smarty.get.email|escape:'html':'UTF-8'}{else}{if isset($smarty.post.email)}{$smarty.post.email|escape:'html':'UTF-8'}{/if}{/if}" />
      </div>
      <p>
        <button type="submit" name="submitGuestTracking" class="btn btn-default btn-md icon-right">
          <span>
            {l s='Send'}
          </span>
        </button>
